...
export default function Page14_PhotoWall() {
  ...
  return (
    <section className="min-h-screen relative py-20 bg-[#1a1a2e] overflow-hidden">
      
      {/* Fairy Lights string (SVG curve) */}
      <div className="absolute top-32 left-0 w-full h-24 pointer-events-none z-0">
        <svg width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 100 100">
          <path d="M0,50 Q25,100 50,50 T100,50" fill="none" stroke="#4a5568" strokeWidth="0.5" />
        </svg>
      </div>

      <div className="text-center mb-24 z-10 relative">
        <h2 className="text-4xl md:text-5xl font-outfit font-bold text-yellow-200 mb-4 drop-shadow-lg">
          The Wall of Fame
        </h2>
      </div>
        ...
        {wallPhotos.map((photo, i) => (
          ...
          >
            ...
            <motion.div 
              ...
              className="bg-white p-3 pb-10 shadow-xl rounded-sm w-40 md:w-56"
            >
              <img src={photo.url} alt="Memory" className="w-full h-40 md:h-56 object-cover bg-gray-200 pointer-events-none" onError={(e) => { e.target.style.background = 'linear-gradient(135deg, #f472b6, #a855f7)'; e.target.src = ''; }} />
            </motion.div>
          </div>
        ))}
      </div>
      ...
      <div className="absolute top-[8rem] md:top-[9rem] left-0 w-full flex justify-around pointer-events-none">
        {[...Array(10)].map((_, i) => (
          <motion.div 
            ...
            className="w-4 h-4 bg-yellow-200 rounded-full blur-[2px] shadow-[0_0_15px_#fef08a]"
        ...
        {selected && (
          ...
          >
            <motion.div
              ...
              className="bg-white p-6 pb-20 rounded-lg max-w-xl w-full shadow-2xl relative"
            >
              ...
              <p className="absolute bottom-6 left-0 right-0 text-center font-outfit text-3xl text-gray-800 font-medium px-4">
                {selected.caption}
              </p>
              <button 
                onClick={() => setSelected(null)}
